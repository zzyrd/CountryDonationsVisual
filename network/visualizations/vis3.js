function vis3(info_data, data, div) {
  const margin = {top: 70, right: 10, bottom: 300, left: 220};
  ...
  const amountExtent = d3.extent(data.map(d=>d.data).flat(), d=>d.amount)
  ...
  let index = 1
  d3.select('#vis3_button').on('click', function(){
        // console.log(index, data[index].year)
        g.selectAll('rect')
        ...
        d3.select('.yearText').text(data[index].year)
